    ...
    <Modal visible={visible} animationType="fade" transparent onRequestClose={onCancel}>
      <View className="flex-1 items-center justify-center bg-black/60 px-6">
        <View className="bg-background-800 rounded-3xl w-full border border-white/10 overflow-hidden">
          ...
          {/* INFO note */}
          {infoNote != null && (
            <View className="mx-4 mb-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl p-3 flex-row items-start gap-2">
              <Ionicons name="information-circle-outline" size={18} color="#60a5fa" />
              <Text className="typo-caption text-blue-300 leading-5 flex-1 mr-2 text-left">
                {infoNote}
              </Text>
            </View>
          )}
