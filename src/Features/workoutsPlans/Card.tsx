...
const Card = ({ plan, selectedIds, toggleSelection, isAlreadyAdded }: Props) => {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <View className="flex-1 ml-3  flex items-start">
        <Text
          numberOfLines={1}
          className={`typo-body-primary text-right ${isSelected ? 'text-white' : 'text-zinc-300'}`}
        >
          {plan.title}
        </Text>
        ...
        <View className="flex-row items-center gap-2.5 mt-1.5 justify-end">
          <View className="flex-row items-center gap-1">
            <Text className="typo-caption text-zinc-500">
          ...
          <View className="flex-row items-center gap-1">
            <Text className="typo-caption text-zinc-500">{plan.time} דק׳</Text>
            <Clock
              size={11}
              color={isSelected ? colors.lime[500] : colors.background[400]}
              strokeWidth={2}
      ...
      {isAlreadyAdded ? (
        <View className="bg-lime-500/20 px-2 py-0.5 rounded-full border border-lime-500/40">
          <Text className="typo-caption-bold text-lime-400">קיים</Text>
        </View>
      ) : (
        <View
