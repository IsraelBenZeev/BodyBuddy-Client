  ...
  ({
    ...
    return (
      <View className="flex-col items-center">
        ...
        {!hideText && <Text className="typo-caption text-lime-500">{text}</Text>}
      </View>
    );
  }
);
const Buttons = ({ plan }: { plan: WorkoutPlan }) => {
  ...
  const { mutate: duplicatePlan, isPending: duplicatePending } = useDuplicateWorkoutPlan(
    user?.id as string
  );
  ...
  return (
    ...
    >
      ...
      <Button
        ...
        icon={
          <MaterialCommunityIcons name="trash-can-outline" size={26} color={colors.lime[500]} />
        }
      />
    </View>
  );
