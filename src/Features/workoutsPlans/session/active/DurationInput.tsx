          ...
          const TimeUnit = ({ amount, unit, step }: { amount: number; unit: string; step: number }) => (
            <View className="flex-1 items-center gap-2">
              ...
              <View className="flex-row items-center rounded-2xl border border-white/5 bg-zinc-900 p-1">
                <AppButton disabled={disabled} onPress={() => updateValue(totalSeconds, step, onChange)} className="rounded-full border border-lime-500/45 bg-lime-500/20 p-2" haptic="medium" animationType="both" accessibilityLabel={`הוסף ${unit}`} hitSlop={6}>
                  <MaterialCommunityIcons name="plus" size={22} color="rgb(213,255,95)" />
                </AppButton>
                <Text className="min-w-[44px] text-center text-3xl font-black text-white">{String(amount).padStart(2, '0')}</Text>
                <AppButton disabled={disabled} onPress={() => updateValue(totalSeconds, -step, onChange)} className="rounded-full border border-white/10 bg-background-800 p-2" haptic="medium" animationType="both" accessibilityLabel={`הפחת ${unit}`} hitSlop={6}>
