...
const Buttons = ({ exerciseId, exerciseName, exerciseName_he }: ButtonsProps) => {
  ...
  const handleGoogle = useCallback(() => {
    Linking.openURL(
      `https://www.google.com/search?q=${encodeURIComponent(exerciseName + ' exercise')}`
    );
  }, [exerciseName]);
  ...
  return (
    <View>
      <View className="flex-row justify-between px-6 mt-2 mb-5 w-full">
        {[
          {
            icon: <IconSearchGoogle size={20} color={colors.lime[500]} />,
            text: 'גוגל',
            onPress: handleGoogle,
          },
          {
            icon: <IconAddToListFitness size={20} color={colors.lime[500]} />,
            text: 'הוסף',
            onPress: handleOpenPlans,
          },
          {
            icon: <IconShare size={20} color={colors.lime[500]} />,
            text: 'שתף',
            onPress: handleShare,
          },
          {
            icon: (
              <Entypo
                ...
                size={20}
                color={isFav ? colors.lime[400] : colors.lime[500]}
              />
            ),
            text: 'מועדף',
          ...
          >
            ...
            <Text className="typo-caption text-zinc-500">{btn.text}</Text>
          </AppButton>
        ))}
      </View>
